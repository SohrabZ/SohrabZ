<div align="center">

# Hi 👋, I'm Sohrab Sheikhani

### Co-Founder & CTO at [Aloware](https://aloware.com)

**San Diego, CA**

I co-founded [Aloware](https://aloware.com) in 2017 and have led its engineering ever since, taking it from a call-tracking tool to an **AI-driven phone system embedded in the CRM**, now used by **1,000+ companies**.

I built the team, the architecture, and most of the early code. I still write the code.

</div>

## What I Lead

**[Aloware](https://aloware.com)** is a cloud contact center for revenue teams. Calling, texting, and AI voice agents, native inside HubSpot and other CRMs. I own the technical direction end to end: telephony, media, AI, and the platform underneath.

- 🏗️ **Co-founded in 2017**, bootstrapped to profitability. Pre-Series A and cash-flow positive
- 📞 **1,000+ companies** running their daily call and SMS volume on the platform
- 🤖 **AI-first since 2023**: SMS bots, autonomous voice agents, and AI receptionists in production
- 🌎 **Distributed engineering org** across the US, LATAM, and Europe. Remote since day one

## How I Work

- 🧭 **Hands-on CTO.** I set architecture, review code, and take on-call. Titles don't exempt anyone from the terminal.
- 🎯 **Small, senior teams.** I'd rather hire five people who own outcomes than twenty who own tickets.
- ⚙️ **Buy the boring, build the edge.** We own our telephony, media, and AI layers because that's where the product lives.
- 📊 **Ship, measure, cut.** Most features should be deleted. The trick is finding out fast.
- 🧪 **Bet early on new primitives.** Real-time voice AI, LLM agents, and MCP were all side projects here before they were roadmap items.

## Open Source from Aloware

| Project | What it is |
| --- | --- |
| [livekit-plugins-dtln](https://github.com/aloware/livekit-plugins-dtln) | Self-hosted noise cancellation for LiveKit. In-process ONNX inference, ~8ms latency, no cloud calls, MIT licensed. A free alternative to LiveKit Cloud + Krisp. [`pip install livekit-plugins-dtln`](https://pypi.org/project/livekit-plugins-dtln/) |

## Still Shipping

Nights and weekends I build small things end to end. It keeps my judgment calibrated when I'm reviewing someone else's design doc.

| Project | What it is |
| --- | --- |
| [swiftui-macos-app](https://github.com/SohrabZ/swiftui-macos-app) | A modern macOS app template in pure SwiftUI. Liquid Glass, adaptive themes, Sparkle updates, no Xcode project |
| [x-browser-mcp](https://github.com/SohrabZ/x-browser-mcp) | Read and post to X from your own logged-in browser, exposed to local AI agents over MCP. No API keys |
| [liquid-glass-skills](https://github.com/SohrabZ/liquid-glass-skills) | Agent skills for building iOS 26 Liquid Glass UIs with SwiftUI + UIKit |
| [stock-tracker](https://github.com/SohrabZ/stock-tracker) | CLI stock trend monitor in Go. Trend/volume signals, position P&L, cron-friendly |
| [buzz-agent-teams](https://github.com/SohrabZ/buzz-agent-teams) | Two ready-to-import agent teams for Buzz: a standing Hive and a disposable Swarm |

## Tech I Reach For

<p align="left">
  <a href="https://www.php.net/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/php-colored.svg" width="36" height="36" alt="PHP" /></a>&nbsp;
  <a href="https://laravel.com/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/laravel-colored.svg" width="36" height="36" alt="Laravel" /></a>&nbsp;
  <a href="https://nodejs.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/nodejs-colored.svg" width="36" height="36" alt="Node.js" /></a>&nbsp;
  <a href="https://www.python.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/python-colored.svg" width="36" height="36" alt="Python" /></a>&nbsp;
  <a href="https://go.dev/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/go-colored.svg" width="36" height="36" alt="Go" /></a>&nbsp;
  <a href="https://www.typescriptlang.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/typescript-colored.svg" width="36" height="36" alt="TypeScript" /></a>&nbsp;
  <a href="https://vuejs.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/vuejs-colored.svg" width="36" height="36" alt="Vue.js" /></a>&nbsp;
  <a href="https://nextjs.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/nextjs-colored.svg" width="36" height="36" alt="Next.js" /></a>&nbsp;
  <a href="https://www.swift.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/swift-colored.svg" width="36" height="36" alt="Swift" /></a>&nbsp;
  <a href="https://www.postgresql.org/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/postgresql-colored.svg" width="36" height="36" alt="PostgreSQL" /></a>&nbsp;
  <a href="https://supabase.com/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/supabase-colored.svg" width="36" height="36" alt="Supabase" /></a>&nbsp;
  <a href="https://aws.amazon.com/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/aws-colored.svg" width="36" height="36" alt="AWS" /></a>&nbsp;
  <a href="https://kubernetes.io/" target="_blank" rel="noreferrer"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/kubernetes-colored.svg" width="36" height="36" alt="Kubernetes" /></a>
</p>

| Category | Technologies |
| --- | --- |
| Voice & Real-Time | Twilio, Telnyx, SignalWire, LiveKit, SIP, WebRTC, media pipelines |
| AI & Agents | Realtime voice agents, LLM orchestration, MCP servers, RAG |
| Backend | PHP/Laravel, Node.js, Python, Go, PostgreSQL, MySQL, Redis |
| Frontend & Mobile | Vue.js, React, Next.js, SwiftUI, iOS, Android |
| Platform | AWS, Kubernetes, Docker, Terraform, Jenkins |
| Prototyping | Supabase, Vercel, Next.js. What I reach for when I want something live by the weekend |

---

## Connect

- 💼 LinkedIn: https://www.linkedin.com/in/sohrabz/
- 🐦 X/Twitter: https://twitter.com/_sohrabz_
- 💻 GitHub: https://github.com/SohrabZ
- 🌐 Website: https://sohrabz.github.io
- 🏢 Aloware: https://aloware.com
- ✉️ Email: sohrab@aloware.com

<sub>Always happy to talk to engineers who want to work on real-time voice AI. <a href="https://aloware.com/careers">We're hiring</a>.</sub>

<!--
This is your GitHub profile README (repo name must be exactly: SohrabZ).
The plain text and links above give Google real content to index on
github.com/SohrabZ instead of the generic auto-description.
-->
